...
class StreamHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/health": return send_json(self, 200, {"status":"ok","service":"axaliai-stream-host","network":NETWORK,"version":VERSION,"brand":BRAND})
        if parsed.path != "/stream": return send_json(self, 404, {"error":"not_found"})
        ...
        decision = route(query); result = plan(query)
        ...
        for event in [{"type":"stream.started","network":NETWORK,"version":VERSION,"brand":BRAND},{"type":"route.completed","mode":decision.get("mode"),"resources":decision.get("resources",[])},{"type":"plan.completed","data":result},{"type":"stream.completed"}]:
            self.wfile.write(("data: "+json.dumps(event)+"\n\n").encode()); self.wfile.flush(); time.sleep(0.05)
    def log_message(self,*_): pass
def main():
    print(f"{BRAND} | {NETWORK} v{VERSION} | {HOST}:{PORT}"); HTTPServer((HOST,PORT),StreamHandler).serve_forever()
