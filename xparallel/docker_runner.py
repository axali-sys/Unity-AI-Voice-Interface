...
def run_in_container(intent: dict) -> dict:
    ...
    try:
        ...
        return {"environment": "docker", "started_at": started, "status": "failed", "error": "container timeout"}
    except OSError as exc:
        return {"environment": "docker", "started_at": started, "status": "failed", "error": str(exc)}

    output = completed.stdout.strip().splitlines()
    success = completed.returncode == 0 and "XP_CONTAINER_OK" in output
    return {
        "environment": "docker", "image": IMAGE, "started_at": started,
        "status": "success" if success else "failed", "exit_code": completed.returncode,
        "evidence": output[-10:], "stderr": completed.stderr[-2000:],
