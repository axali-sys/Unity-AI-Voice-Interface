...
def _deterministic_simulation(intent: dict) -> dict:
    ...
    return {
        ...
        "evidence": ["intent accepted", "deterministic sandbox simulation completed"] if success else ["empty intent"],
        "implementation": {"type": "simulation-only", "goal": goal},
        "limitations": ["Docker was unavailable, so no real container was started", "Simulation success is not production proof"],
    }
...
def simulate(intent: dict) -> dict:
    ...
    if docker_available():
        ...
        if result.get("status") in {"success", "failed"}:
            result.setdefault("limitations", ["Container probe is not production proof", "Real-world execution is disabled in V0.1"])
            return result
